...
function getID($tableName){
	...
	$maxID = mysqli_query( $conn, $maxQuery) or trigger_error(mysqli_error()." Query was: ".$maxQuery);
	while($infoMaxID = mysqli_fetch_array( $maxID )){
		$id = $infoMaxID['maxID'];
	}
	$id = $id+1;
	return $id;
...
mysqli_query($conn, $sql_user)or trigger_error(mysqli_error()." Query was: ".$sql_user);

//Begin code added by Jochem Douw (25 June 2012)
//Code overwrites default behaviour: user_id should contain the real added ID in stead of the generated one
...
mysqli_query($conn, $sql_consultation_inst)or trigger_error(mysqli_error()." Query was: ".$sql_consultation_inst);
...
function submit_response($table_info, $consultation){
	...
	$consultationQuery = "select practical_reasoning_as from consultation where consultation_id=$consultation";
	$dataPRAS =mysqli_query($conn, $consultationQuery) or trigger_error(mysqli_error());
	$dataPRASInfo = mysqli_fetch_array($dataPRAS);
			...
			if(isset($submit_circ)){
				...
				foreach($submit_circ as $circumstance){
					...
					$sql_circumstance = "INSERT INTO proposition_prur
						...
						mysqli_query($conn, $sql_circumstance)or trigger_error(mysqli_error()." Query was: ".$sql_circumstance);
					...
					mysqli_query($conn, $sql_csur) or trigger_error(mysqli_error());
				}
		...
		case "consequence_prur":
			...
			if(isset($submit_cons)){
				foreach($submit_cons as $consequence){
					...
					mysqli_query($conn, $sql_consequence) or trigger_error(mysqli_error());
		...
		case "value_prur":
			...
			if(isset($submit_value)){
				...
				foreach($submit_value as $value){
					...
					mysqli_query($conn, $sql_value)or trigger_error(mysqli_error()." Query was: ".$sql_value);
					...
					mysqli_query($conn, $sql_csur)or trigger_error(mysqli_error()." Query was: ".$sql_csur);
					...
					mysqli_query($conn, $sql_vrur)or trigger_error(mysqli_error()." Query was: ".$sql_vrur);
				}
			}else{
